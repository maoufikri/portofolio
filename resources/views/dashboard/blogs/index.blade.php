...
@section('content')
    <div class="container mx-auto px-4 py-6">
        <div class="flex justify-between items-center border-b pb-3">
            <h1 class="text-2xl font-bold text-gray-700">My Blogs</h1>
            <!-- Tambah Portofolio Button -->
            <a href="{{ route('dashboard.blogs.create') }}" class="bg-blue-500 text-white px-4 py-2 rounded-lg shadow-md hover:bg-blue-600">
                Add New Blog
            </a>
        </div>
    </div>
            ...
            <table class="min-w-full border-collapse border border-gray-200">
                <thead>
                    <tr class="bg-gray-100">
                        ...
                        <th class="px-6 py-3 text-left text-sm font-medium text-gray-600 border border-gray-200">content</th>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="px-6 py-4 text-sm text-gray-700 border border-gray-200">{{ $blog->title }}</td>
                        <td class="px-6 py-4 text-sm text-gray-700 border border-gray-200">{{ \Illuminate\Support\Str::limit($blog->content, 50) }}</td>
                        <td class="px-6 py-4 text-sm text-gray-700 border border-gray-200">
                            <img src="{{ asset('storage/' . $blog->image) }}" alt="Image" class="w-12 h-12 rounded-full">
                        </td>
                        <td class="px-6 py-4 text-sm text-gray-700 border border-gray-200">
                            <!-- Edit -->
                            ...
                            <!-- Hapus -->
                            <form action="{{ route('dashboard.blogs.destroy', $blog->id) }}" method="POST" class="inline-block ml-2">
                                @csrf
                                @method('DELETE')
                                <button 
                                type="submit" 
